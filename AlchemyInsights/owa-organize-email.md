---
title: Organizar el correo electrónico en Outlook en la Web
ms.author: daeite
author: daeite
manager: joallard
ms.date: 04/21/2020
ms.audience: Admin
ms.topic: article
ms.service: o365-administration
ROBOTS: NOINDEX, NOFOLLOW
localization_priority: Normal
ms.custom: ''
ms.openlocfilehash: 6f776d830fc3e017c8611353f31af8f0164f2f0873d01d0d6bc5bc5f5c92eef5
ms.sourcegitcommit: b5f7da89a650d2915dc652449623c78be6247175
ms.translationtype: MT
ms.contentlocale: es-ES
ms.lasthandoff: 08/05/2021
ms.locfileid: "53983110"
---
# <a name="organize-your-email"></a>Organizar el correo electrónico

En Outlook en la Web, puede usar comandos como  **Barrido** y Mover a para eliminar o mover automáticamente mensajes de un remitente determinado. Estos comandos aparecen encima de la lista de mensajes al seleccionar un mensaje en la bandeja de entrada.

- **Barrido** <img src='data:image/png;base64,' />
 permite eliminar rápidamente mensajes en la bandeja de entrada. Puede eliminar automáticamente todo el correo electrónico entrante de ese remitente, conservar solo el correo electrónico más reciente y eliminar el resto, o eliminar el correo electrónico de más de 10 días.
- **Mover a** <img src='data:image/png;base64,' /> le permite mover un mensaje específico a otra carpeta.

Para obtener más información, vea [Trabajar con carpetas de](https://support.office.com/article/ae0f10d6-54e7-4f29-acd3-78cdc3fdcb9f)mensajes en Outlook en la Web .